...
def sce(
    task_tensors: List[torch.Tensor],
    weights: torch.Tensor,                       # ← 第二個參數占位
    density: float = 1.0,
    majority_sign_method: Literal["total", "frequency"] = "total",
) -> torch.Tensor:
    """
    Select-Calculate-Erase merge
    """
    # -------- S: Select --------
    task_tensors = torch.stack(task_tensors, dim=0)  # [T, ...]
    if density < 1.0:
        var_mask = sce_mask(task_tensors, density, mask_dtype=torch.bool)
        task_tensors = task_tensors * var_mask.unsqueeze(0)

    # -------- E: majority sign --------
    if majority_sign_method == "total":
        sign_sum = task_tensors.sum(dim=0)
    else:  # "frequency"
        sign_sum = torch.sign(task_tensors).sum(dim=0)

    sign_majority = torch.sign(sign_sum)                   # +1/-1/0
    agreement = (torch.sign(task_tensors) == sign_majority).float()

    # -------- C: energy weight (可改為直接用傳入 weights) --------
    energy_w = sce_weight(task_tensors)                    # [T]
    energy_w = energy_w.view([-1] + [1]*(task_tensors.dim()-1))

    masked   = task_tensors * agreement
    weighted = masked * energy_w                           # 仍用 energy 權重
    weight_total = (agreement * energy_w).sum(dim=0)
    merged = weighted.sum(dim=0) / weight_total.clamp(min=1e-6)
    return merged
