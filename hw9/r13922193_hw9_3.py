    ...
    def _check_new_adapter_config(self, config: LoraConfig) -> None:
        ...
        if config.bias != "none":
            for existing_config in self.peft_config.values():
                if existing_config.bias != "none":
                    raise ValueError(
                        f"{self.__class__.__name__} supports only one adapter with bias. "
                        f"Found an existing adapter with bias={existing_config.bias}, so new adapter must set bias='none'."
                    )
